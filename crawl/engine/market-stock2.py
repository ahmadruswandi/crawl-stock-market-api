...
def get_info_segment(seglist, segment_text):
    b_segment = False
    segment_arr = []
...
def get_business_reg(seglist):
    br = get_info_segment(seglist, "business registration")
...
def get_financial_data(main_tbl):
...
def fetch_company_profile(company_data):
    try:
        buffer = BytesIO()
        c = pycurl.Curl()
    ...
    financial_data = get_financial_data(main_tbl)

    company_profile_td = main_tbl.find_all("td")[0]

    seglist = get_segment_list(main_tbl)
    auditing_company = get_info_segment(seglist, "auditing company")
    business_registration = get_business_reg(seglist)
    business_summary = get_info_segment(seglist, "business summary")
...
class MarketStockEngine:
    ...
    def fetch_company_list(self):
        ...
        # with pool as p:
        #     records = p.map(fetch_company_profile, datalist)
...
datalist = mse.fetch_company_list()
# p = pp.ProcessPool(4)
# records = p.map(fetch_company_profile, datalist)
# print('type(records)', type(records))
